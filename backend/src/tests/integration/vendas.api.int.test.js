...
describe('API /api/vendas', () => {
  ...
  test('POST cria venda e atualiza status do celular', async () => {
    ...
    const res = await request(app).post('/api/vendas').send(payload).expect(201);
  ...
  test('DELETE remove venda e libera celular', async () => {
    ...
    await request(app).delete(`/api/vendas/${createRes.body.id}`).expect(204);
    await request(app).get(`/api/vendas/${createRes.body.id}`).expect(404);
  ...
  test('POST falha quando celular já vendido', async () => {
    ...
    await request(app).post('/api/vendas').send(payload).expect(201);
    await request(app)
      .post('/api/vendas')
